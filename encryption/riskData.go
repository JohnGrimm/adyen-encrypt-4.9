package encryptions

import (
	"encoding/json"
	"fmt"
	"strings"
	"time"
)

type RiskData struct {
	UserAgent           string
	Language            string
	ColorDepth          int
	DeviceMemory        int
	HardwareConcurrency int
	Width               int
	Height              int
	AvailWidth          int
	AvailHeight         int
	TimezoneOffset      int
	Timezone            string
	Platform            string
	CpuClass            *string
	DoNotTrack          *string
}

type RiskDataOutput struct {
	Version           string         `json:"version"`
	DeviceFingerprint string         `json:"deviceFingerprint"`
	PersistentCookie  []string       `json:"persistentCookie"`
	Components        JsonComponents `json:"components"`
}

type Component struct {
	Key   string
	Value interface{}
}

type JsonComponents struct {
	UserAgent             string `json:"userAgent"`
	Webdriver             int    `json:"webdriver"`
	Language              string `json:"language"`
	ColorDepth            int    `json:"colorDepth"`
	DeviceMemory          int    `json:"deviceMemory"`
	PixelRatio            int    `json:"pixelRatio"`
	HardwareConcurrency   int    `json:"hardwareConcurrency"`
	ScreenWidth           int    `json:"screenWidth"`
	ScreenHeight          int    `json:"screenHeight"`
	AvailableScreenWidth  int    `json:"availableScreenWidth"`
	AvailableScreenHeight int    `json:"availableScreenHeight"`
	TimezoneOffset        int    `json:"timezoneOffset"`
	Timezone              string `json:"timezone"`
	SessionStorage        int    `json:"sessionStorage"`
	LocalStorage          int    `json:"localStorage"`
	IndexedDB             int    `json:"indexedDB"`
	AddBehavior           int    `json:"addBehavior"`
	OpenDatabase          int    `json:"openDatabase"`
	Platform              string `json:"platform"`
	Plugins               string `json:"plugins"`
	Canvas                string `json:"canvas"`
	AdBlock               int    `json:"adBlock"`
	HasLiedLanguages      int    `json:"hasLiedLanguages"`
	HasLiedResolution     int    `json:"hasLiedResolution"`
	HasLiedOs             int    `json:"hasLiedOs"`
	HasLiedBrowser        int    `json:"hasLiedBrowser"`
	Fonts                 string `json:"fonts"`
	Audio                 string `json:"audio"`
	EnumerateDevices      string `json:"enumerateDevices"`
}

func padString(f string, e int) string {
	if len(f) >= e {
		return f[:e]
	}
	padding := strings.Repeat("0", e-len(f))
	return padding + f
}

func processDFPComponents(components []Component) map[string]interface{} {
	parsedComponents := make(map[string]interface{})

	for _, component := range components {
		key := component.Key
		val := component.Value

		// Verifica se é um slice
		switch v := val.(type) {
		case []string:
			if key == "screenResolution" {
				parsedComponents["screenWidth"] = v[0]
				parsedComponents["screenHeight"] = v[1]
				continue
			}
			if key == "availableScreenResolution" {
				parsedComponents["availableScreenWidth"] = v[0]
				parsedComponents["availableScreenHeight"] = v[1]
				continue
			}
			val = strings.Join(v, "")
		case []int:
			if key == "screenResolution" {
				parsedComponents["screenWidth"] = v[0]
				parsedComponents["screenHeight"] = v[1]
				continue
			}
			if key == "availableScreenResolution" {
				parsedComponents["availableScreenWidth"] = v[0]
				parsedComponents["availableScreenHeight"] = v[1]
				continue
			}
		}

		// Verifica valores especiais
		if val == "not available" || val == "error" || val == "excluded" {
			continue
		}

		// Verifica tipo do valor
		switch v := val.(type) {
		case bool:
			if v {
				parsedComponents[key] = 1
			} else {
				parsedComponents[key] = 0
			}
		case int, float64:
			parsedComponents[key] = v
		case string:
			if key == "timezone" || key == "language" || key == "platform" || key == "webglVendorAndRenderer" {
				parsedComponents[key] = v
			} else {
				parsedComponents[key] = X64hash128(v, 0)
			}
		}

		if key == "plugins" {
			parsedComponents[key] = "29cf71e3d81d74d43a5b0eb79405ba87"
		}
	}

	return parsedComponents
}

func NewRiskData(userAgent, language string, colorDepth, deviceMemory, hardwareConcurrency, width, height, availWidth, availHeight, timezoneOffset int, timezone, platform string, cpuClass, doNotTrack *string) *RiskData {
	return &RiskData{
		UserAgent:           userAgent,
		Language:            language,
		ColorDepth:          colorDepth,
		DeviceMemory:        deviceMemory,
		HardwareConcurrency: hardwareConcurrency,
		Width:               width,
		Height:              height,
		AvailWidth:          availWidth,
		AvailHeight:         availHeight,
		TimezoneOffset:      timezoneOffset,
		Timezone:            timezone,
		Platform:            platform,
		CpuClass:            cpuClass,
		DoNotTrack:          doNotTrack,
	}
}

func (rd *RiskData) dfValue() string {
	return fmt.Sprintf("%s:40", rd.generateFingerprint())
}

func (rd *RiskData) generateComponents() JsonComponents {
	components := []Component{
		{"userAgent", rd.UserAgent},
		{"webdriver", false},
		{"language", rd.Language},
		{"colorDepth", rd.ColorDepth},
		{"deviceMemory", rd.DeviceMemory},
		{"pixelRatio", 2},
		{"hardwareConcurrency", rd.HardwareConcurrency},
		{"screenResolution", []int{rd.Width, rd.Height}},
		{"availableScreenResolution", []int{rd.AvailWidth, rd.AvailHeight}},
		{"timezoneOffset", rd.TimezoneOffset},
		{"timezone", rd.Timezone},
		{"sessionStorage", true},
		{"localStorage", true},
		{"indexedDb", true},
		{"addBehavior", false},
		{"openDatabase", false},
		{"cpuClass", "not available"},
		{"platform", rd.Platform},
		{"doNotTrack", "not available"},
		{"plugins", [][][]interface{}{
			{
				{"PDF Viewer", "Portable Document Format", [][]string{
					{"application/pdf", "pdf"},
					{"text/pdf", "pdf"},
				}},
			},
		}},
		{"canvas", []string{
			"canvas winding:yes",
			"canvas fp:data:image/png;base64,",
		}},
		{"webgl", "not available"},
		{"webglVendorAndRenderer", nil},
		{"adBlock", false},
		{"hasLiedLanguages", false},
		{"hasLiedResolution", false},
		{"hasLiedOs", false},
		{"hasLiedBrowser", false},
		{"fonts", []string{
			"Andale Mono", "Arial", "Arial Black", "Arial Hebrew", "Arial Narrow",
			"Arial Rounded MT Bold", "Arial Unicode MS", "Calibri", "Comic Sans MS",
			"Courier", "Courier New", "Geneva", "Georgia", "Helvetica",
			"Helvetica Neue", "Impact", "LUCIDA GRANDE", "Microsoft Sans Serif",
			"Monaco", "Palatino", "Tahoma", "Times", "Times New Roman",
			"Trebuchet MS", "Verdana", "Wingdings", "Wingdings 2", "Wingdings 3",
		}},
		{"audio", "124.04344968475198"},
		{"enumerateDevices", []string{
			"id=;gid=;audioinput;", "id=;gid=;videoinput;", "id=;gid=;audiooutput;",
		}},
	}

	// Serializa o mapa para JSON
	jsonData, err := json.Marshal(processDFPComponents(components))
	if err != nil {
		return JsonComponents{}
	}

	// Desserializa o JSON para a struct
	var JComponent JsonComponents
	err = json.Unmarshal(jsonData, &JComponent)
	if err != nil {

		return JsonComponents{}
	}

	return JComponent
}

func (rd *RiskData) generateFingerprint() string {
	result := make(map[string]string)
	s := map[string]int{
		"plugins":       10,
		"nrOfPlugins":   3,
		"fonts":         10,
		"nrOfFonts":     3,
		"timeZone":      10,
		"video":         10,
		"superCookies":  10,
		"userAgent":     10,
		"mimeTypes":     10,
		"nrOfMimeTypes": 3,
		"canvas":        10,
		"cpuClass":      5,
		"platform":      5,
		"doNotTrack":    5,
		"webglFp":       10,
		"jsFonts":       10,
	}

	// Plugins
	pluginsObj := "Plugin 0: Chrome PDF Viewer; Portable Document Format; internal-pdf-viewer; (Portable Document Format; application/pdf; pdf) (Portable Document Format; text/pdf; pdf). Plugin 1: Chromium PDF Viewer; Portable Document Format; internal-pdf-viewer; (Portable Document Format; application/pdf; pdf) (Portable Document Format; text/pdf; pdf). Plugin 2: Microsoft Edge PDF Viewer; Portable Document Format; internal-pdf-viewer; (Portable Document Format; application/pdf; pdf) (Portable Document Format; text/pdf; pdf). Plugin 3: PDF Viewer; Portable Document Format; internal-pdf-viewer; (Portable Document Format; application/pdf; pdf) (Portable Document Format; text/pdf; pdf). Plugin 4: WebKit built-in PDF; Portable Document Format; internal-pdf-viewer; (Portable Document Format; application/pdf; pdf) (Portable Document Format; text/pdf; pdf). "
	result["plugins"] = padString(CalculateMd5_b64(pluginsObj), s["plugins"])
	result["nrOfPlugins"] = padString("5", s["nrOfPlugins"])

	// Fonts
	result["fonts"] = padString("", s["fonts"])

	// nrOfFonts
	result["nrOfFonts"] = padString("", s["nrOfFonts"])

	// Time Zone
	// c := 240 // Example: timezone offset in minutes
	// d := 300

	// Cria uma nova data com o dia 1
	e := time.Date(time.Now().Year(), 1, 1, 0, 0, 0, 0, time.Local)

	// Define o mês para maio (mês 5)
	e = time.Date(e.Year(), time.Month(5), 1, 0, 0, 0, 0, e.Location())
	c := e.UTC().Sub(e).Minutes() // Obtém o timezone offset em minutos

	// Define o mês para dezembro (mês 11)
	e = time.Date(e.Year(), time.Month(11), 1, 0, 0, 0, 0, e.Location())
	d := e.UTC().Sub(e).Minutes() // Obtém o timezone offset em minutos

	timeZoneData := fmt.Sprintf("%v**%v", c, d)
	result["timeZone"] = padString(CalculateMd5_b64(timeZoneData), s["timeZone"])

	// Video
	videoData := (rd.Width + 7) * (rd.Height + 7) * rd.ColorDepth
	result["video"] = padString(fmt.Sprintf("%d", videoData), s["video"])

	// Super Cookies
	superCookiesData := CalculateMd5_b64("DOM-LS: No, DOM-SS: No") + CalculateMd5_b64(", IE-UD: No")
	result["superCookies"] = padString(superCookiesData, s["superCookies"])

	// User Agent
	result["userAgent"] = padString(CalculateMd5_b64(rd.UserAgent), s["userAgent"])

	// Mime Types
	mimeTypesData := "undefinedPortable Document Formatapplication/pdfpdfPortable Document Formattext/pdfpdf"
	result["mimeTypes"] = padString(CalculateMd5_b64(mimeTypesData), s["mimeTypes"])
	result["nrOfMimeTypes"] = padString("2", s["nrOfMimeTypes"])

	// Canvas
	canvasData := "data:image/png;base64,"
	result["canvas"] = padString(CalculateMd5_b64(canvasData), s["canvas"])

	// cpuClass

	if *rd.CpuClass == "" {
		result["cpuClass"] = padString("", s["cpuClass"])
	} else {
		result["cpuClass"] = padString(CalculateMd5_b64(*rd.CpuClass), s["cpuClass"])
	}

	// platform
	if rd.Platform == "" {
		result["platform"] = padString("", s["platform"])
	} else {
		result["platform"] = padString(CalculateMd5_b64(rd.Platform), s["platform"])
	}

	// doNotTrack
	if *rd.DoNotTrack == "" {
		result["doNotTrack"] = padString("", s["doNotTrack"])
	} else {
		result["doNotTrack"] = padString(CalculateMd5_b64(*rd.DoNotTrack), s["doNotTrack"])

	}

	// jsFonts
	jsFontsData := "Wingdings 3;Wingdings 2;Wingdings;Webdings;Verdana;Univers CE 55 Medium;Trebuchet MS;Times New Roman;Times;Tahoma;Symbol;Rockwell;PT Serif;PT Sans;Papyrus;Palatino;Modern No. 20;Microsoft Sans Serif;LUCIDA GRANDE;Impact;Humanst 521 Cn BT;Helvetica Neue;Helvetica;Goudy Bookletter 1911;Gill Sans;GeoSlab 703 XBd BT;GeoSlab 703 Lt BT;Georgia;Futura;Exo 2;English 111 Vivace BT;Courier New;Courier;Copperplate;Comic Sans MS;Calibri;Bookshelf Symbol 7;Bodoni 72 Smallcaps;Bodoni 72 Oldstyle;Bodoni 72;Bauhaus 93;Baskerville;Arial Unicode MS;Arial Rounded MT Bold;Arial Narrow;Arial Hebrew;Arial Black;Arial;Andale Mono;American Typewriter"
	result["jsFonts"] = padString(CalculateMd5_b64(jsFontsData), s["jsFonts"])

	// webGl
	result["webglFp"] = padString(CalculateMd5_b64("0000000000"), s["webglFp"])

	for i := range result {
		if len(result[i]) != s[i] {
			result[i] = padString("", s[i])
		}
	}

	f := result["plugins"] + result["nrOfPlugins"] + result["fonts"] + result["nrOfFonts"] + result["timeZone"] + result["video"] + result["superCookies"] + result["userAgent"] + result["mimeTypes"] + result["nrOfMimeTypes"] + result["canvas"] + result["cpuClass"] + result["platform"] + result["doNotTrack"] + result["webglFp"] + result["jsFonts"]
	f = strings.ReplaceAll(f, "+", "G")
	f = strings.ReplaceAll(f, "/", "D")
	return f
}

func (rd *RiskData) Generate() (string, error) {
	fingerprint := rd.dfValue()

	output := RiskDataOutput{
		Version:           "1.0.0",
		DeviceFingerprint: fingerprint,
		PersistentCookie:  []string{},
		Components:        rd.generateComponents(),
	}

	jsonData, err := json.Marshal(output)
	if err != nil {
		return "", err
	}

	return string(jsonData), nil
}
